...
public class StickyHeaderDecoration extends RecyclerView.ItemDecoration {
	...
	public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
		...
		if (position != RecyclerView.NO_POSITION
			...
			View header = getHeader(parent, position).getViewHolder().itemView;
	...
	public View findHeaderViewUnder(float x, float y) {
		for (ViewHolder holder : mHeaderCache.values()) {
			final View child = holder.getViewHolder().itemView;
	...
	private ViewHolder getHeader(RecyclerView parent, int position) {
		...
		if (mHeaderCache.containsKey(key)) {
			...
			vh.setViewHolder(holder);
			vh.setMaxViewHeight(header.getMeasuredHeight());

			mHeaderCache.put(key, vh);
		...
		for (int layoutPos = 0; layoutPos < count; layoutPos++) {
			...
			if (adapterPos != RecyclerView.NO_POSITION && hasHeader(adapterPos)) {
				...
				if (headerId != previousHeaderId) {
					...
					View header = getHeader(parent, adapterPos).getViewHolder().itemView;
					canvas.save();
	...
	private View setHeaderViewHeight(RecyclerView parent, View headerView, int height) {
		...
		int childHeight = ViewGroup.getChildMeasureSpec(heightMeasureSpec,
	...
	private int getHeaderTop(RecyclerView parent, View child, View header, int adapterPos, int layoutPos) {
		...
		if (layoutPos == 0) {
			...
			// Loop over adapter children
			for (int i = 1; i < count; i++) {

				// Establish adapter posiiton of child
				int adapterPosHere = parent.getChildAdapterPosition(parent.getChildAt(i));
				if (adapterPosHere != RecyclerView.NO_POSITION) {

					// Get header ID for child
					long nextId = mAdapter.getHeaderId(adapterPosHere);

					if (nextId != currentId) {
						// This child has a different header from the previous one.
						final View next = parent.getChildAt(i);

						// Get Y Position of this child
						int nextChildYPos = (int) next.getY();

						// Get the next header view holder
						RecyclerView.ViewHolder nextHeaderVH = getHeader(parent, adapterPosHere).getViewHolder();

						// Get the height of this next header
						int nextHeaderHeight = nextHeaderVH.itemView.getHeight();

						// Calculate if top header should be pushed off screen.
						final int offset = nextChildYPos - (headerHeight + nextHeaderHeight);
						...
						if (offset < 0) {
							...
							// Header collapsed, check if header can be expanded
							int proposedHeight = headerHeight + Math.abs(offset);
							int newHeight = Math.min(proposedHeight, getHeader(parent, adapterPosHere).getMaxViewHeight());
							setHeaderViewHeight(parent, header, newHeight);
							return 0;
						}
						return offset;
	...
	private static class ViewHolder {
		...
		public void setMaxViewHeight(int maxViewHeight) {
